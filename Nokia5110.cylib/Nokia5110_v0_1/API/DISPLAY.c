...
#endif // DRAW_OVER_BACKGROUND
{
    ...
    `$SPI_PERIPHERAL`_Start();          // Enable SPI
#if defined(CY_SCB_`$SPI_PERIPHERAL`_H) // SCB based SPI
    `$SPI_PERIPHERAL`_SpiSetActiveSlaveSelect(`$SPI_PERIPHERAL`_SPI_SLAVE_SELECT0);
#endif
...
/**
* Description : SetPointer to background display.
...
*               dt (available only with DRAW_OVER_BACKGROUND directive) ->
* drawing type
* Return value : None.
*/
#ifdef DRAW_OVER_BACKGROUND
void `$INSTANCE_NAME`_Chr(char ch, DRAW_TYPE_t dtype, FONT_TYPE_t ft)
{
    ...
    case Small_Font: // Small font
        ...
        `$INSTANCE_NAME`_SendData(
            Background[Background_cursor]); // empty space after character
        break;
    case Middle_Font:
    default:
        if (dtype == DRAW_OR) {
...
void `$INSTANCE_NAME`_Chr(char ch, FONT_TYPE_t ft)
{
    ...
    case Small_Font:
        for (uint8_t i = 0; i < 3; i++) {
            `$INSTANCE_NAME`_SendData(
                SmallFont[ch - 32][i]); // empty space after character
        }
        `$INSTANCE_NAME`_SendData(0x00);
        break;
    case Middle_Font:
    default:
        for (uint8_t i = 0; i < 5; i++) {
            `$INSTANCE_NAME`_SendData(
                MiddleFont[ch - 32][i]); // empty space after character
        }
        `$INSTANCE_NAME`_SendData(0x00);
        break;
    }
...
*               yposition -> bargraph position (by pixel, must  be <=
(yend-ystart+1)*8)
*               dt (available only with DRAW_OVER_BACKGROUND directive) ->
drawing type
* Return value : None.
...
*               xposition -> bargraph position (must  be xstart <= xposition <=
*xend)
*               dt (available only with DRAW_OVER_BACKGROUND directive) ->
*drawing type
* Return value : None.
